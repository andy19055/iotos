      ...
      <Card>
        ...
        <Table ref="table"
            ...
            <template v-for="colItem in columns" slot-scope="{ row, index }" :slot="colItem.slot">
              <Select ref="selecter+" v-if="editIndex === index && colItem.filter.type === 'Select'" :value="row[colItem.slot]" :name="colItem.slot" @on-open-change="optionselected(colItem.slot)" @on-change="classChoose" readonly>
                <Option v-for="(value,key) in colItem.filter.option" v-if="key != 0" :value="value.name">{{value.name}}</Option>
              </Select>
              <Input v-else-if="editIndex === index && colItem.filter.type === 'Input'" type="text" v-model="editData[colItem.slot]"/>
            ...
            <template slot-scope="{ row, index }" slot="action">
              ...
              <div v-else>
                <Button @click="handleEdit(row, index)"><Icon type="ios-construct" size="large" style="margin-right: 5px"></Icon>操作</Button>
              </div>
            </template>
    ...
    methods: {
      ...
      handleSave (index) {
        ...
        for(var key in this.editData)
          this.data[index][key] = this.editData[key]
        this.editIndex = -1;
      },
      exportExcel () {
        ...
        this.data = this.datasource.filter(item => {
          ...
          if(this.datasource.length > 0) {
            for (var key in this.datasource[0]) {
              sumtmp += (Boolean(this.search[key]) && item[key].toUpperCase().indexOf(this.search[key].toUpperCase()) == -1)
            }
    ...
    created() {
      for (let index in this.columns) {
        ...
        if (this.columns[index].filter) {
          ...
          }else if (this.columns[index].filter.type && this.columns[index].filter.type === 'Button') {
            render = (h) => {
              return h(this.columns[index].filter.type, {
                props: {
                  icon: 'ios-search',
